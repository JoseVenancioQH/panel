...
$pdf->Write(0, 'Estado Financiero Fianzas', '', 0, 'L', true, 0, false, false, 0);
...
$PagosFianzas = $this->reportes_fianzas_module->get_buscar_fianzas($liststatusfianza,
                                                                   $id_operador,
                                                                   $fianza_afectado,
                                                                   $fianza_fecha,
                                                                   $fianza_tabulador,
                                                                   $reportes_fianzasFechas);
...
if(!empty($PagosFianzas)){  
  ...
  foreach($PagosFianzas as $rowPagosFianzas){                      
      ...
      if($banfiltro){     
            $cadConcat .= "<tr style=\"background-color:#d0dbf1;\">";
            $cadConcat .= "<td>".$rowPagosFianzas->reg_pagosfianzas_id_pagosfianzas."</td>";                     
            $cadConcat .= "<td>".$rowPagosFianzas->cat_vincularfianza_noconvenio."</td>";
            $cadConcat .= "<td>".$rowPagosFianzas->cat_statusfianzas_nombre."</td>";
            $cadConcat .= "<td>$".number_format($rowPagosFianzas->cat_vincularfianza_montopactado,2)."</td>";
            $cadConcat .= "<td>".$rowPagosFianzas->cat_vincularfianza_fecharegistro."</td>";
            $cadConcat .= "<td>".$rowPagosFianzas->cat_operadores_nombre."</td>";
            $cadConcat .= "<td>".$rowPagosFianzas->cat_vincularfianza_nombreafectado."</td>";                         
            $cadConcat .= "<td>$".number_format($rowPagosFianzas->reg_pagosfianzas_monto_suma,2)." (".$rowPagosFianzas->numreg.") "."</td>";                          
            $cadConcat .= "<td>$".number_format($deuda,2)."</td></tr>";
            $cadConcat .= "<tr><th style=\"background-color:#d0dbf1;\" colspan=\"9\" align=\"center\">Pagos Realizados</th></tr>";

            foreach(json_decode("[".$rowPagosFianzas->pagos."]",true) as $key=>$pago){
                $cadConcat .= "<tr class=\"reportes_fianzas_pagos pagos_fianzas_".$rowPagosFianzas->reg_pagosfianzas_id_pagosfianzas."\">";
                $cadConcat .= "<td>".(isset($pago['consecutivo'])?$pago['consecutivo']:"")."</td>";                     
                $cadConcat .= "<td>$".(is_numeric($pago['monto'])?number_format($pago['monto'],2):"")."</td>";
                $cadConcat .= "<td>".(isset($pago['fecha'])?$pago['fecha']:"")."</td>";
                $cadConcat .= "<td></td>";
                $cadConcat .= "<td></td>";
                $cadConcat .= "<td></td>";
                $cadConcat .= "<td></td>";                              
      ...
      $cadConcat .= "<tr><th colspan=\"9\" align=\"center\">Pagos Realizados</th></tr>";
