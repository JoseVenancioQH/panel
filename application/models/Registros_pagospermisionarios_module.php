...
Class Registros_PagosPermisionarios_Module extends CI_Model {
	...
	public function get_buscar_permisionarios($id_permisionario=null,$id_unidad=null,$id_operador=null,$id_placa=null,$tabulador=null,/*$fianzasStatus=null,*/$permisionarioFechas=null,$vincular_fechainicio=null) {
		...
		$filtro[] = "vp.cat_vincularpermisionario_status = 1";
		...
		$this->db->select('
						   ...
						   SUM(if(`rpp`.`reg_pagospermisionarios_tipo` = 1 ,`reg_pagospermisionarios_monto_cobro`,0)) as reg_pagospermisionarios_monto_cobro,
						   count(*) as numreg
					  	');
		...
		//$this->db->where('rpp.reg_pagospermisionarios_tipo=1');
		if(!empty($tabulador)){$this->db->limit($tabulador);}
		if(!empty($condition)){$this->db->where($condition);}
		$this->db->group_by("vp.cat_vincularpermisionario_id_vincularpermisionario"); //excelente
	...
	public function get_registros_pagospermisionarios($table,$tabulador) {
		...
		$this->db->join('cat_cobros as co', 'vp.cat_vincularpermisionario_id_cobros = co.cat_cobros_id_cobros', 'left');
		$this->db->join('reg_pagospermisionarios as rpp', 'vp.cat_vincularpermisionario_id_vincularpermisionario = rpp.reg_pagospermisionarios_id_vincularpermisionario', 'left');
		$this->db->where('vp.cat_vincularpermisionario_status = 1');
		if(!empty($tabulador)){$this->db->limit($tabulador);}
	...
	public function registros_pagospermisionarios_insert($data) {
		...
		if ($this->db->affected_rows() > 0) {
			$this->logs_controlplacas_module->logs_controlplacas("success","insert","reg_pagospermisionarios",$this->session->userdata['logged_in']['idusuario'],$data);
			return true;
		} else {
			$this->logs_controlplacas_module->logs_controlplacas("error","insert","reg_pagospermisionarios",$this->session->userdata['logged_in']['idusuario'],$data);
			return false;
		}
	...
	public function regsitros_pagospermisionarios_update($data,$id) {
		...
		if ($this->db->affected_rows() > 0) {
			$this->logs_controlplacas_module->logs_controlplacas("success","update","reg_pagospermisionarios",$this->session->userdata['logged_in']['idusuario'],$data);
			return true;
		} else {
			$this->logs_controlplacas_module->logs_controlplacas("error","update","reg_pagospermisionarios",$this->session->userdata['logged_in']['idusuario'],$data);
			return false;
		}
	...
	public function registros_pagospermisionarios_delete($data,$idpagospermisionarios) {
		...
		if ($this->db->affected_rows() > 0) {
			$this->logs_controlplacas_module->logs_controlplacas("success","delete","reg_pagospermisionarios",$this->session->userdata['logged_in']['idusuario'],$data);
			return true;

		} else {
			$this->logs_controlplacas_module->logs_controlplacas("error","delete","reg_pagospermisionarios",$this->session->userdata['logged_in']['idusuario'],$data);
			return false;
